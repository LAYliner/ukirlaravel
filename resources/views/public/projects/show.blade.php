...
@section('title', $project->title . ' - Sanggar Ukir Tana Paser')
        ...
        @auth
            <div class="mb-10 bg-white p-6 rounded-lg border border-secondary/30 shadow-sm">
                <form action="{{ route('comments.store') }}" method="POST">
                    ...
                    <div class="mb-4">
                        <textarea name="content" rows="3" placeholder="Tulis pendapat Anda tentang karya ini..." class="w-full p-3 bg-background border border-secondary/50 rounded text-text placeholder-text/50 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all" required></textarea>
                    </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                @foreach($relatedProjects as $relProject)
                    <a href="{{ route('projects.show', $relProject->slug) }}" class="group block bg-white border border-secondary/30 rounded-lg overflow-hidden hover:border-accent/50 transition-all duration-300 shadow-sm hover:shadow-md">
                        ...
                        <div class="p-4">
                            <h4 class="text-lg font-bold text-text group-hover:text-primary transition-colors line-clamp-2">{{ $relProject->title }}</h4>
                        </div>
                    </a>
                @endforeach
            </div>
