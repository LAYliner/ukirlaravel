...
@section('content')
    <div class="card">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
            <h2>📝 Daftar Blog</h2>
            <a href="{{ route('admin.blog.create') }}" style="padding: 0.5rem 1rem; background: var(--accent-color); color: #fff; text-decoration: none; border-radius: 4px;">
                + Buat Baru
            </a>
        </div>
            ...
            <tbody>
                @forelse($blogs as $blog)
                    <tr style="border-bottom: 1px solid var(--border-color);">
                        <td style="padding: 1rem;">{{ $blog->judul }}</td>
                        <td style="padding: 1rem;">
                            <span style="padding: 0.25rem 0.5rem; border-radius: 4px; font-size: 0.85rem; background: {{ $blog->status_blog === 'publish' ? '#d4edda' : '#fff3cd' }}; color: {{ $blog->status_blog === 'publish' ? '#155724' : '#856404' }};">
                                {{ ucfirst($blog->status_blog) }}
                            </span>
                        </td>
                        <td style="padding: 1rem;">{{ $blog->created_at->format('d M Y, H:i') }}</td>
                        <td style="padding: 1rem;">{{ $blog->admin->nama ?? 'N/A' }}</td>
                        <td style="padding: 1rem;">
                            <a href="{{ route('admin.blog.edit', $blog->id_blog) }}" style="color: var(--accent-color); text-decoration: none; margin-right: 0.5rem;">Edit</a>
                            <form method="POST" action="{{ route('admin.blog.destroy', $blog->id_blog) }}" style="display: inline;" onsubmit="return confirm('Hapus blog ini?')">
                                @csrf
                                @method('DELETE')
